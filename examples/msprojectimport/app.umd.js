...
function lowerCaseConfig(data) {
  ...
  Object.keys(data).forEach(function (key) {
    config[bryntum.gantt.StringHelper.lowercaseFirstLetter(key)] = data[key];
  });
  return config;
}
...
var project = window.project = new bryntum.gantt.ProjectModel({
  ...
  onClick: function onClick() {
    ...
    if (files) {
      ...
      bryntum.gantt.AjaxHelper.post('php/load.php', formData, {
        parseJson: true
      }).then(
      /*#__PURE__*/
      function () {
            ...
            while (1) {
              switch (_context.prev = _context.next) {
                ...
                case 6:
                  input.clear();
                  gantt.unmaskBody();
                  bryntum.gantt.Toast.show('File imported successfully!');
                  _context.next = 12;
                  break;
  ...
  insertFirst: document.getElementById('tools') || document.body
    ...
    input = _bryntum$gantt$Widget2[0],
    sendBtn = _bryntum$gantt$Widget2[1];
