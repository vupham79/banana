  ...
  methods: {
    ...
    checkGridSanity: function checkGridSanity(grid) {
      ...
      if (!grid.hideHeaders) {
        for (i = 0; i < rowCount; i++) {
          ...
          for (var col = 0; col < columnCount; col++) {
            ...
            var validDelta = this.browser.msie || this.browser.msedge ? 2 : 0;
      ...
      if (!(navigator.userAgent.match(/trident/i) && grid.element.style.transform.indexOf('scale') > -1)) {
        var _validDelta = this.browser.msie || this.browser.msedge ? 2 : 1.5;

        if (Math.abs(grid.bodyHeight - grid.bodyContainer.getBoundingClientRect().height) > _validDelta) {
    ...
    generateGridByPages: function generateGridByPages() {
      ...
      var grid = new this.global.Grid({
        ...
        features: {
          pdfExport: {
            exportServer: '/export',
            headerTpl: function headerTpl() {
              return "<div style=\"height:".concat(headerHeight, "px;background-color: grey\">HEAD</div>");
            },
    ...
    getExportHtml: function () {
      var _getExportHtml = _asyncToGenerator(
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee(grid, exportConfig) {
        var result;
      ...
      function getExportHtml(_x, _x2) {
        return _getExportHtml.apply(this, arguments);
      }
