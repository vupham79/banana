    ...
    methods : {
        ...
        async createGanttForExport({
            ...
            const
                ...
                { tasksData, dependenciesData } = await this.global.ProjectGenerator.generateAsync(verticalPages * (rowsPerPage - 1), rowsPerPage - 1, () => {}),
        ...
        assertExportedGanttDependenciesList(doc, dependencies = [], { fromSide, toSide } = {}, arrowMargin = 12) {
            ...
            function getToSide(doc, dependency) {
                ...
                if (dependency.toSide) {
                    ...
                    let from = dependency.fromEvent,
                        fromTime = from.endDate.getTime(),
                        fromIndex = doc.querySelector(`.b-timeline-subgrid [data-id="${from.id}"]`).dataset.index,
                        to = dependency.toEvent,
                        toTime = to.startDate.getTime(),
                        toIndex = doc.querySelector(`.b-timeline-subgrid [data-id="${to.id}"]`).dataset.index;
            ...
            dependencies.forEach(dep => {
                let from         = dep.sourceEvent,
                    to           = dep.targetEvent;
        
                if (from && to) {
                    // firefox is case sensitive to this
                    let dependencyEl = doc.querySelector(`[depid="${dep.id}"]`),
        ...
        assertExportedTasksList(t, doc, tasks = []) {
            let pass = true;
            ...
            tasks.forEach(task => {
                ...
                if (!taskElement) {
                    t.fail(`Element is not found for task ${task.name || task.id}`);
                    pass = false;
                }
                else {
                    const
                    ...
                    if (rowBox.intersect(taskBox).height !== taskBox.height) {
                        t.fail(`Task ${task.id} is not aligned to row`, {
                            got      : taskBox,
                            need     : rowBox,
