StartTest(t => {
    ...
    t.it('Selecting single tasks after creating new one should work', (t) => {
        t.chain(
            ...
            async() => t.selectorCountIs('.b-tree-cell.b-selected', 1, 'One row is selected'),
    ...
    t.it('Reset data after TaskEditor is shown should work', (t) => {
        t.chain(
            ...
            { click : '.b-menuitem :textEquals(Edit)', desc : 'Clicked task editor menu entry' },
            { type : '[ESCAPE]' },
            clickResetButton,
            ...
            { rightClick : '[data-task-id=11]', desc : 'Right clicked the task' },
            { click : '.b-menuitem :textEquals(Edit)', desc : 'Clicked task editor menu entry' },
            { click : '.b-popup .b-button:textEquals(Cancel)', desc : 'Clicked "Cancel"' },
            clickResetButton,
    ...
    t.it('Should not duplicate newly created and saved task', (t) => {
        t.chain(
            { click : tools.addTaskButton.element },
            clickSaveButton,
            async() =>  t.selectorCountIs('.b-sch-label:contains(New task)', 1, 'Only one new task is rendered')
        );
    });
    ...
    t.it('Should not throw exception when opening features menu', (t) => {
        t.chain(
            ...
            async() =>  t.elementIsNotVisible('.b-menuitem :contains(Show baselines)', 'No baselines entry')
        );
    });
