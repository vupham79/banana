...
*/
import { Panel, AllColumns, ProjectModel, Gantt, DateHelper } from '../../build/gantt.module.js?437987';
...
StartTest(t => {
    ...
    t.it('Should not show emptyText while loading', t => {
        const project = new ProjectModel({
            transport : {
                load : {
        ...
        // No empty text before loading starts
        t.selectorNotExists('.b-grid-empty');

        const async = t.beginAsync();

        project.load().then(() => {
            // No empty text after load
            t.selectorNotExists('.b-grid-empty');

            t.endAsync(async);
        });

        // No empty text during load
        t.selectorNotExists('.b-grid-empty');
    });
